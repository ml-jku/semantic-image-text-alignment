...
def create_parser():
    ...
    parser.add_argument('--fraction', type=float, default=1.0, help="Fraction of data to use for training of the mapping")
    parser.add_argument('--dataset', type=str, required=True, choices=["coco", "flickr30k"], default='coco', help="Fraction of data to use for training of the mapping")
...
def main():
    ...
    outfile = open(f'./benchmark_times.log', 'a')
    ...
    for encoder in encoders:
        ...
        if not os.path.exists(f'./data/{encoder_clean}_{lm}_{dataset}_{options.fraction}_embs.npy'):
            ...
            np.save(f'./data/{encoder_clean}_{lm}_{dataset}_{options.fraction}_embs', src_embs)
            np.save(f'./data/{encoder_clean}_{lm}_{dataset}_{options.fraction}_targets', tar_embs)
            np.save(f'./data/{encoder_clean}_{lm}_{dataset}_{options.fraction}_train_cls', train_cls)
        else:
            src_embs = np.load(f'./data/{encoder_clean}_{lm}_{dataset}_{options.fraction}_embs.npy')
            tar_embs = np.load(f'./data/{encoder_clean}_{lm}_{dataset}_{options.fraction}_targets.npy')
            train_cls = np.load(f'./data/{encoder_clean}_{lm}_{dataset}_{options.fraction}_train_cls.npy')

        # tar_embs are already centered and scaled
        outfile.write(f"{encoder_clean}:\n")
        src_embs = (src_embs - src_embs.mean(0)) / src_embs.std(0)
        src_embs = src_embs[train_cls]

        for train_method in ['linear_reg', 'ridge', 'procrustes', 'robust_procrustes']:
            ...
            print(f"Train Method: {train_method}, Time: {np.mean(times)} +- {np.std(times)}")
            outfile.write(f"Train Method: {train_method}, Time: {np.mean(times)} +- {np.std(times)}")
